...
export class CartController {
  ...
  async IncrementCartItem(Req: Request, Res: Response) {
    try {
      ...
      let result = await this.cartService.IncrementCartItem(
        getUserIdFromToken(Req as ExtendedRequest),
  ...
  async DecrementCartItem(Req: Request, Res: Response) {
    try {
      ...
      let result = await this.cartService.DecrementCartItem(
        getUserIdFromToken(Req as ExtendedRequest),
        Req.params.CartId as string,
      );
  ...
  async DeleteCart(Req: Request, Res: Response) {
    try {
      ...
      let result = await this.cartService.DeleteCart(
        getUserIdFromToken(Req as ExtendedRequest),
        Req.params.CartId as string,
      );
  ...
  async GetUserCarts(Req: Request, Res: Response) {
    try {
      ...
      let result = await this.cartService.GetUserCarts(getUserIdFromToken(Req as ExtendedRequest));
  ...
  async ClearCart(Req: Request, Res: Response) {
    try {
      let result = await this.cartService.ClearCart(getUserIdFromToken(Req as ExtendedRequest));
      return Res.status(200).json(result);
    } catch (error) {
      return Res.status(500).json(ServiceResponse.failure<object>(ErrorCode.SERVER, error instanceof Error ? error.message : "an internal server error occured."));
    }
